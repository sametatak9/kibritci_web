const fs = require('fs');
const path = './src/components/IdariScreen.tsx';
...
content = content.replace(tabsOriginal, tabsReplaced);

// 3. Add Personel View Logic
const viewOriginal = `          {kampMainView === 'faaliyet' ? (
            <KampFaaliyetTakipTab />
          ) : (
        <div className="flex-1 flex flex-col lg:flex-row gap-4 lg:gap-6 min-h-0">`;

const viewReplaced = `          {kampMainView === 'faaliyet' ? (
            <KampFaaliyetTakipTab />
          ) : kampMainView === 'personel' ? (
...
content = content.replace(viewOriginal, viewReplaced);

// 4. Update Firma Özeti in Left Panel
const statsOriginal = `                <div className="flex justify-between">
...
const statsReplaced = `                <div className="flex justify-between">
                  <span>Toplam Yatak Kapasitesi:</span>
                  <strong className="text-slate-800">
                    {kampOdalari.reduce((acc, current) => acc + current.kapasite, 0)} Yatak
                  </strong>
...
content = content.replace(statsOriginal, statsReplaced);

// 5. Update Floor Title & Bird's Eye View
const floorOriginal = `                        <div className="flex justify-between items-center bg-slate-100 p-2 rounded-lg border-l-4 border-amber-500">
...
content = content.replace(floorOriginal, floorReplaced);

fs.writeFileSync(path, content, 'utf8');
console.log("IdariScreen patched successfully.");
