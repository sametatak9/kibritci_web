...
3. **Taşeron grup (Arnavutköy İşe Giriş):** Ayrı WhatsApp grubu resmi olarak takip edilir; bot gruba giremez. Her mesaj bir PDF’dir: `AD SOYAD İŞE GİRİŞ BİLDİRGESİ.pdf` + alt yazı (`Yurt mekanik giriş`) veya `TC_ayrilis.pdf`. PDF Grup Köprüsü → **Taşeron grup**’a bırakılır. Parser yön + firma + ad/TC okur. Onaylayınca **tek** kuyruk kaydı (`kaynak: TASERON_GRUP`) açılır. Kadro bu ekrandan yazılmaz. Onay’da taşeron kartı `gorev: TAŞERON PERSONEL` ile açılır; çıkış onaylanana kadar pasif yok. Ana Firma kartına dokunulmaz. Haftalık **Taşeron Liste Güncelle** ayrı kalır.
