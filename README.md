...
3. **Taşeron grup (Arnavutköy İşe Giriş):** Ayrı WhatsApp grubu resmi olarak takip edilir; bot gruba giremez. Her mesaj bir SGK e-Bildirge PDF’idir (`SİGORTALI İŞE GİRİŞ BİLDİRGESİ` veya `SİGORTALI İŞTEN AYRILIŞ BİLDİRGESİ`): `AD SOYAD İŞE GİRİŞ BİLDİRGESİ.pdf` + alt yazı (`Yurt mekanik giriş`) veya `TC_ayrilis.pdf`. PDF Grup Köprüsü → **Taşeron grup**’a bırakılır. Sunucu form metninden ad / soyad / TC / meslek / işveren / tarihi okur (yapay zeka yalnızca boş kalan alanlar için). PDF’deki uzun ünvan kurulu taşeron cari/kart adına hizalanır (`YURTMEKANİK…` → `YURT MEKANİK`). Onaylayınca **tek** kuyruk kaydı (`kaynak: TASERON_GRUP`) açılır. Kadro bu ekrandan yazılmaz. Onay’da giriş taşeron kartı `gorev: TAŞERON PERSONEL` ile açılır. Çıkış yalnızca programdaki **TC** eşleşirse taşeron kartı pasife alır; TC yoksa kadroya dokunulmaz. Ana Firma kartına dokunulmaz. Haftalık **Taşeron Liste Güncelle** ayrı kalır.
